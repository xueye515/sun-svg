...
const Start = {
    ...
    _formatData: function (data) {
        const { nodes, links, centerPoint, groupList } = data;
        links.forEach(link => {
        ...
        return { nodes, links, centerPoint, groupList }
    },
    init: function (svgEle, allData) {
        const { backGround, dataList } = allData;
        const { nodes, links, centerPoint, groupList } = this._formatData(backGround);

        this._nodes = nodes;
        this._links = links;
        this._centerPoint = centerPoint;
        this._groupList = groupList;
        this._darkHorse = this._formatData(dataList[dataList.length - 1]);
        this._svgEle = svgEle;
    },
    draw: function () {
        ...
        this._panZoomTiger = svgPanZoom(this._svgEle, { onZoom: this._onZoomFnc });//设置缩放
        ...
        this._drawFilter(svg_filter_g);//遮罩层
        ...
        const svg_darkHorse_g = document.createElementNS(svgNameSpace, 'g');
        svg_darkHorse_g.setAttribute('class', "start-darkHorse-info");//黑马竞品
        svgCon.appendChild(svg_darkHorse_g);
    },
    //初始框，控制初缩放
    ...
    _drawStarts: function (svg_nodes, starts) {
        starts.forEach(singleStart => {
            ...
            //绑定鼠标悬浮事件
            this._bindStartMouse(svg_circle);
    ...
    showGroupInfo: function (groupList) {
        ...
        this._groupList.forEach(group => {
            const svg_group = document.createElementNS(svgNameSpace, 'polygon');
            svg_group.setAttribute('class', "start-group");
            let points = '';
            group.positionList.forEach(point => {
                points += `${point.row * 1},${point.col * 1} `
            })
            svg_group.setAttribute('points', points);
            // svg_group.setAttribute('style', "fill:rgb(255,0,0);fill-opacity:0.9;pointer-events: none;");
            start_Group_g.appendChild(svg_group);
        })
    },
    ...
    hideGroupInfo: function () {
        const start_Group_g = document.querySelector('.start-group-info');
        for (var j = 0; j < start_Group_g.childElementCount;) {
            start_Group_g.removeChild(start_Group_g.firstChild);
        }
    },

    //锁定黑马
    lockDarkHouse: function () {
        const svg_darkHorse_g = document.querySelector('.start-darkHorse-info');
        const { nodes, links, centerPoint, groupList } = this._darkHorse;


        if (this._panZoomTiger) {
            this._panZoomTiger.zoomAtPoint(3, { x: nodes[0].row, y: nodes[0].col });
        }

        this._drawLinks(svg_darkHorse_g, links);//划线
        this._drawStarts(svg_darkHorse_g, nodes);//划点
    ...
    //取消锁定黑马
    unlockDarkHouse: function () {

        const svg_darkHorse_g = document.querySelector('.start-darkHorse-info');
        for (var j = 0; j < svg_darkHorse_g.childElementCount;) {
            svg_darkHorse_g.removeChild(svg_darkHorse_g.firstChild);
        }

        if (this._panZoomTiger) {
            this._panZoomTiger.resetZoom();
            this._panZoomTiger.resetPan();
        }
        const svg_filter = document.querySelector('.start-filter');
        svg_filter.setAttribute('style', "fill:rgb(255,255,255);fill-opacity:0;pointer-events: none;");
